...
class ACGAN():
    ...
    def test(self, path):
        ...
        if not os.path.exists('gp_ac_samples'):
            os.makedirs('gp_ac_samples')
        for i in range(5):
            ...
            for (c,s) in zip(ID, samples):
                scipy.misc.imsave('gp_ac_samples/sample_{}_{}.png'.format(c, i+1), s)
...
if __name__ == '__main__':
    ...
    # run
    acgan = ACGAN(generator, discriminator, train_X, train_y, test_y, 'gp_ac_ckpt')
    if sys.argv[1] == 'test':
